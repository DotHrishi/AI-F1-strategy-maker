...
for year, track, event in SAMPLE_RACES:
    try:
        ...
        # Get Gemini analysis for significant differences
        if time_delta > 10 or pit_match == 0:
            print(f"  🤖 Getting AI analysis for {year} {track}...")
            actual_data = {
                'track': track,
                'weather': weather,
                'strategy': actual_strat,
                'pit_laps': actual_pits,
                'total_time': actual_time
            }
            predicted_data = {
                'track': track,
                'weather': weather,
                'strategy': [(f'COMPOUND_{i+1}', len(pred_pits)+1) for i in range(len(pred_pits)+1)],
                'pit_laps': pred_pits,
                'total_time': pred_time
            }
            
            analysis = gemini.compare_strategies(actual_data, predicted_data)
            if analysis and "Error" not in analysis:
                print(f"  📊 AI Insights: {analysis[:200]}...")  # Show first 200 chars
            else:
                print(f"  ⚠️ AI analysis unavailable")
...
# Generate overall insights using Gemini
if successful_validations > 0:
    print(f"\n🤖 Generating AI insights on validation results...")
    validation_summary = {
    ...
    overall_insights = gemini.generate_race_insights(validation_summary)
    if overall_insights and "Error" not in overall_insights:
        print(f"\n📊 GEMINI AI INSIGHTS:")
        print("="*60)
        ...
        with open('ai_insights.txt', 'w', encoding='utf-8') as f:
            f.write("F1 Strategy AI - Gemini Analysis\n")
            f.write("="*50 + "\n\n")
            f.write(f"Validation Summary:\n")
            f.write(f"- Successful validations: {successful_validations}/{len(SAMPLE_RACES)}\n")
            f.write(f"- Average time error: {np.mean(metrics['time_delta']):.1f}%\n")
            f.write(f"- Pit strategy match rate: {np.mean(metrics['strat_match'])*100:.0f}%\n\n")
            f.write("Gemini AI Analysis:\n")
            f.write("-" * 30 + "\n")
            f.write(overall_insights)
        
        print(f"\n💾 AI insights saved to ai_insights.txt")
    else:
        print(f"\n⚠️ Could not generate AI insights")
